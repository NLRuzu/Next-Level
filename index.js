  ...
  if(message.content.toUpperCase().startsWith("+VERIFICAR")){
        message.delete();
            if(message.member.roles.find("name", "🌟 STAFF NIVEL 3") || message.member.roles.find("name", "🌟 STAFF NIVEL 2") ||message.member.roles.find("name", "🌟 STAFF NIVEL 1")){
				...
				if(message.author.voiceChannel != null || message.author.voiceChannel != undefined){
					...
					if(desc != null){
						...
						message.author.voiceChannel.createInvite(options)
						.then(invite => 
							...
							description: "Busco **" + 4 - message.author.voiceChannel.members.size + "** soldados en el autobús **" + message.author.voiceChannel.name + "** para luchar.",
							fields: [{
								name: "Descripción",
								value: "*" + desc + "*0"
							},
